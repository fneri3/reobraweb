...
const criaNovaLoja = (nome, descricao, preco, estoque, id) => {
    const linhaLojaC = document.createElement('li')
    const conteudo = `
        ...
        linhaLojaC.innerHTML = conteudo
        linhaLojaC.dataset.id = id

    return linhaLojaC
}

const tabela = document.querySelector('[data-lojaC]')

const render = async () => {
    const listaLojas = await servicosLojas.lojaC()
...
tabela.addEventListener('click', (evento) => {
    ...
    if(id == 1){
        ...
        quantidade++
        precoD = 83 * quantidade
    } else {
        quantidade++
        precoE = 55 * quantidade
    }
})
